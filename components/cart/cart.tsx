    ...
    return (
        <Link href="/cart"  passHref>
            <div className="z-50 fixed rounded-full bottom-2 right-2 flex items-center opacity-50 cursor-pointer transform duration-500 hover:scale-105 hover:opacity-100">
                {cart?.products?.length && <div className="absolute z-30 bottom-2 right-2 text-center bg-white bg-opacitity-50 text-xs w-5 h-5 rounded-full">{cart?.products?.length}</div>}
